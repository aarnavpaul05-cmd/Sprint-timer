...
+  autoNext: true, mode: 'beep', practice: false, vibrate: true, abortController: null, unlocked: false, beforeInstallPrompt: null,
 };
...
+function sayOrBeep(text){ const m=state.mode; if(m==='voice'){ const ok=speak(text); if(!ok) beepFor(text);} else if(m==='beep'||m==='practice'){ beepFor(text);} }
...
+    lightGun.classList.add('on'); setCue('GUN!'); sayOrBeep('Gun!'); if(state.vibrate&&navigator.vibrate) navigator.vibrate(60); state.gunAt=performance.now(); if(state.practice) finishPracticeTrial();
   }catch(e){ cleanupAfterTrial(); }
 }
+function finishPracticeTrial(){ if(!state.practice||!state.running) return; state.trial+=1; state.lastMs=null; updateStats(); log(`Trial ${state.trial}: Practice run complete (no timing) ℹ️`,'meta'); setStatus('Practice trial complete.'); setCue('Tap Start for next'); cleanupAfterTrial(); autoMaybeNext(); }
...
+tapArea.addEventListener('pointerdown', ()=>{ if(!state.running||state.practice) return; if(state.awaitingGun && state.gunAt===0) falseStart(); else if(state.awaitingGun && state.gunAt>0) finishValidReaction(); });
